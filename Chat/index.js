    ...
    <View style={styles.view}>
          ...
          mensagens.length > 0 && mensagens.map(item => (
            ...
            <View key={item.id} style={styles.linha_conversa}>
              ...
              <View style={{ flexDirection: 'column', marginTop: 5 }}>
                ...
                {typeof (item.mensagem) == "string" && !(item.mensagem).includes('alt=media&token') ? <Text>{item.mensagem}</Text> : <Text> </Text>}
                {(item.mensagem) == "string" ? <Image style={styles.img} source={{ uri: item.mensagem }} /> : <Image style={styles.img} source={{ uri: item.mensagem }} />}
              </View>
      ...
      <View style={styles.footer}>
        ...
        <TouchableOpacity onPress={escolherImagem}>
          <Ionicons style={{ margin: 3 }} name="md-apps" size={32} color={'#999'} />
        </TouchableOpacity>
